...
export function StudentForm() {
  ...
  const uploadFileToCloudinary = async (): Promise<string | null> => {
    if (!file) return null;
    setUploading(true);

    const cloudinaryData = new FormData();
    cloudinaryData.append('file', file);
    cloudinaryData.append('upload_preset', 'edu_upload'); // Replace with your preset

    try {
      const response = await axios.post(
        'https://api.cloudinary.com/v1_1/YOUR_CLOUD_NAME/upload', // Replace YOUR_CLOUD_NAME
        cloudinaryData
      );
      setUploading(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (file) {
      uploadedFileUrl = await uploadFileToCloudinary();
      if (!uploadedFileUrl) {
        // Stop submission if upload failed
        return;
      }
    }
            ...
            {step === 3 && (
              <div className="space-y-6">
                ...
                <div>
                  ...
                  >
                    Upload a document or picture (optional)
                  </label>
                  <input
                    type="file"
                    id="fileUpload"
                  ...
                  />
                  {uploading && <p className="text-sm text-blue-600 mt-2">Uploading file...</p>}
                  {file && !uploading && (
